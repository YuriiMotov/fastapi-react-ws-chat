...
async def test_get_first_circle_user_list__success(
    ...
    """
    get_first_circle_user_list() returns list of users in chats, where this user
    is a member.
    """
    user_1 = u_c["user_1"]
    ...
    first_circle_user_list = await chat_manager.get_first_circle_user_list(
...
async def test_get_first_circle_user_list__empty_list(
    ...
    """
    get_first_circle_user_list() returns empty list if user doesn't have chats
    """
    user_1 = u_c["user_1"]
    ...
    assert len(first_circle_user_list) == 0
...
async def test_get_first_circle_user_list__repo_failure(
    ...
    """
    get_first_circle_user_list() raises RepositoryError in case of repository failure.
    """
    user_1 = u_c["user_1"]
    await chat_manager.add_user_to_chat(
        current_user_id=user_1.id, user_id=u_c["user_1"].id, chat_id=u_c["chat_1"].id
    ...
    with patch.object(
        ...
        # Call get_first_circle_user_list() and check that it raises RepositoryError
